 ...
 */
export const getLineMessageQuota = onRequest(
  {
    cors: [
      "http://localhost:5173",
      "http://localhost:5174",
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (statsDoc.exists) {
        ...
        appointmentNotifications = data?.appointmentNotifications || 0;
        completionNotifications = data?.completionNotifications || 0;
        reminderNotifications = data?.reminderNotifications || 0;
      }
...
async function recordMessageSent(
  shopId: string,
  messageType: "appointment" | "completion" | "reminder"
) {
  try {
    const now = new Date();
    const year = now.getFullYear();
 ...
 */
export const sendServiceCompletionNotification = onRequest(
  {
    cors: true,
    region: "asia-east1",
  },
    ...
    try {
      ...
      const flexMessage = {
        ...
        messages: [
          {
            type: "flex",
            altText: `${appointment?.petName || "您的寶貝"}的服務已完成！`,
            contents: {
              type: "bubble",
              body: {
                type: "box",
 ...
 */
export const sendLineCompletionMessage = onRequest(
  {
    cors: true,
    region: "asia-east1",
  },
 ...
 */
export const sendLineTempReportMessage = onRequest(
  {
    cors: true,
    region: "asia-east1",
  },
 ...
 */
export const declineAppointment = onRequest(
  {
    cors: true,
    region: "asia-east1",
  },
...
export const lineWebhook = onRequest(
  {
    cors: false, // LINE Webhook 不需要 CORS
    region: "asia-east1",
    timeoutSeconds: 60,
  },
  async (req, res) => {
    ...
    try {
      ...
      for (const event of webhookData.events) {
        ...
        if (event.type === "follow") {
          ...
          // 查找對應的商家
          const shopInfo = await findShopForFollowEvent(userId, destination);
          ...
          const { shopId, channelAccessToken } = shopInfo;
          ...
          try {
            ...
            try {
              ...
              if (welcomeDoc.exists) {
                ...
                if (welcomeData?.isActive && welcomeData?.message) {
                  // 發送歡迎訊息
                  const replyPayload = {
                  ...
                  if (replyResponse.ok) {
                    ...
                    logger.error("歡迎訊息發送失敗", {
                      status: replyResponse.status,
                      error: errorText,
                    });
                  }
                } else {
                  logger.info("歡迎訊息未啟用或未設定", { shopId });
                }
              }
            } catch (welcomeError) {
              logger.error("取得或發送歡迎訊息時發生錯誤", {
                error: welcomeError,
              });
              // 不影響主流程，繼續執行
            }
        ...
        if (event.type === "unfollow") {
          ...
          // 查找用戶所屬的商家（通過查詢 users collection）
        ...
        const shopData = shopDoc.data();
        const channelAccessToken = shopData?.lineChannelAccessToken;
        const channelSecret = shopData?.lineChannelSecret;
 ...
 */
async function findShopIdByUserId(userId: string): Promise<string | null> {
  try {
...
/**
 * Helper function: 根據 LINE User ID 和 destination 查詢店鋪 ID
 * 用於 follow/unfollow 事件，透過嘗試取得用戶 Profile 來識別商家
 */
async function findShopForFollowEvent(
  userId: string,
  destination: string
): Promise<{ shopId: string; channelAccessToken: string } | null> {
  try {
    // 取得所有商家
    const shopsSnapshot = await db.collection("shops").get();
    ...
    // 嘗試每個商家的 Channel Access Token
    for (const shopDoc of shopsSnapshot.docs) {
      const shopData = shopDoc.data();
      const token = shopData.lineChannelAccessToken;

      if (!token) continue;

      try {
        // 嘗試使用此商家的 token 取得用戶資料
        const profileResponse = await fetch(
          `https://api.line.me/v2/bot/profile/${userId}`,
          {
            method: "GET",
 ...
 * 用於測試或手動執行
 */
export const manualCheckExpiredSubscriptions = onRequest(
  { cors: true },
  async (req, res) => {
    // 檢查授權（可選）
